...
TokenPosition GetClosingParenthesis(char **buffer, int BUFFER_SIZE,
                                    int START_POSITION, TokenPosition token);
...
ParsingTreeNode *Parse(char **buffer, int BUFFER_SIZE) {
  ...
  base->parent = base;
  base->statement = (Statement){STATEMENT_TYPE_SCOPE, OPERATOR_TYPE_NULL,
                                argument_from_string("Main Context", 12)};
  ...
  for (int i = 0; i < BUFFER_SIZE; i++) {
    ...
    if (currentScope->lookForKeywordOrOperator) {
      // Operators
      if (c == '+') {
        currentScope->statement = (Statement){
            STATEMENT_TYPE_OPERATOR, OPERATOR_TYPE_ADDITION, "Addition"};
      } else if (c == '-') {
        currentScope->statement = (Statement){
            STATEMENT_TYPE_OPERATOR, OPERATOR_TYPE_SUBTRACT, "Subtraction"};
      } else if (c == '*') {
        currentScope->statement = (Statement){
            STATEMENT_TYPE_OPERATOR, OPERATOR_TYPE_MULTIPLY, "Multiplication"};
      } else if (c == '/') {
        currentScope->statement = (Statement){
            STATEMENT_TYPE_OPERATOR, OPERATOR_TYPE_DIVISION, "Division"};
      } else if (c == ' ') {
        currentScope->statement =
            (Statement){STATEMENT_TYPE_SCOPE, OPERATOR_TYPE_NULL, "Scope"};
      } else {
  ...
  scopeDetection:
    if (c == '(') {
      ...
      //   currentScope->statement = (Statement) { STATEMENT_TYPE_SCOPE,
      //   OPERATOR_TYPE_NULL, "Scope" }; lookForKeywordOrOperator = 0;
      // }
      ...
      if (currentScope->lookForArguments) {
        if (currentScope->arg_lhs.argument_type == ARGUMENT_TYPE_NULL) {
          currentScope->arg_lhs =
              (Argument){ARGUMENT_TYPE_FUNCTION, NULL, "Scope", newScope};
        } else if (currentScope->arg_rhs.argument_type == ARGUMENT_TYPE_NULL) {
          currentScope->arg_rhs =
              (Argument){ARGUMENT_TYPE_FUNCTION, NULL, "Scope", newScope};
          currentScope->lookForArguments = 0;
        } else {
          printf("arg_lhs & arg_rhs is non-NULL when they shouldn't be.\n");
      ...
      // printf("parentheses pair: %d, %d\n", currentScope->start_paren.index,
      // closeParenthesis.index);

      // Set statement for scope to be a non-behavioured scope if no statement
      // has been set when reaching the closing parenthesis.
      ...
      // Case: There are more closing than starting parentheses, syntax error.
        ...
        printf("SYNTAX ERROR: More closing parentheses than opening "
               "parentheses.\n");
      }
      // Case: There are equal number of closing and starting parentheses, root
      // scope.
      else if (numOfClosingParentheses == numOfStartingParentheses) {
        currentScope = currentScope->parent;
        numOfClosingParentheses--;
